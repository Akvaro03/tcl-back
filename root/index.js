...
db.serialize(function () {
  ...
  db.run(
    "CREATE TABLE IF NOT EXISTS Activities   (id INTEGER PRIMARY KEY, name TEXT,score NUMERIC,time NUMERIC, users TEXT, state TEXT)"
  );
  db.run(
    "CREATE TABLE IF NOT EXISTS Factura      (id TEXT, dateCreated NUMERIC,dateExpiration NUMERIC, datePay NUMERIC, state TEXT)"
  );
  db.run(
    "CREATE TABLE IF NOT EXISTS TypeOt       (id INTEGER PRIMARY KEY, abbreviation TEXT, nameType TEXT, activities TEXT)"
  );
  db.run(
    "CREATE TABLE IF NOT EXISTS Config       (id INTEGER PRIMARY KEY, nameCompany TEXT,browserLogo TEXT,companyLogo TEXT)"
  );
  db.run(
    "CREATE TABLE IF NOT EXISTS Users        (id INTEGER PRIMARY KEY, name TEXT, type TEXT, email TEXT, password TEXT, state TEXT)"
  );
  db.run(
    "CREATE TABLE IF NOT EXISTS OT           (id INTEGER PRIMARY KEY,nLacre TEXT, contractName TEXT, priority TEXT, OTKey TEXT, Client TEXT,Date NUMERIC, Producto TEXT, Marca TEXT, Modelo TEXT, NormaAplicar TEXT, Cotizacion TEXT, FechaVencimiento DATETIME, FechaEstimada DATETIME, Type TEXT, Description TEXT, StateProcess TEXT, Observations TEXT, Contact TEXT, Changes TEXT, Auth TEXT, Activities TEXT, IdClient NUMERIC, Availability TEXT, Factura TEXT)"
  );
  db.run(
    "CREATE TABLE IF NOT EXISTS Contract     (id INTEGER PRIMARY KEY, name TEXT, url TEXT)"
  );
  db.run(
    "CREATE TABLE IF NOT EXISTS Suggestions  (Observations TEXT,NormaAplicar TEXT)"
  );
  db.run(
    "CREATE TABLE IF NOT EXISTS Clients      (id INTEGER PRIMARY KEY, idEditable NUMERIC,Name TEXT,Document TEXT, KeyUnique TEXT, Contacts TEXT, location TEXT)"
  );
  db.run(
    "CREATE TABLE IF NOT EXISTS newClients   (id NUMERIC, Name TEXT,Document TEXT, KeyUnique TEXT, Contacts TEXT, location TEXT)"
  );
    ...
    function (err, row) {
      if (err) {
        ...
        encryptPassword("123456789").then((data) => {
          db.run(
            `INSERT INTO Users (name, type, email, password, state) VALUES ('Admin', 'Administrador del sistema', 'admin@gmail.com', ?, 'active')`,
            [String(data)],
            function (err) {
              if (err) {
                console.error(err.message);
...
app.post("/getOtKey", (req, res) => {
  ...
  db.serialize(async function () {
    db.all(
      `SELECT * FROM OT WHERE Date = ${dateFormated}`,
      function (err, row) {
        if (err) {
          res.json(err);
        }
...
app.post("/postClients", (req, res) => {
  ...
  db.serialize(async function () {
    db.run(
      "INSERT INTO Clients (idEditable, Name, Document, KeyUnique, Contacts, location) VALUES (?,?,?,?,?,?)",
      [idEditable, Name, Document, KeyUnique, Contacts, location],
...
app.post("/postUsers", (req, res) => {
  ...
  encryptPassword(password).then((data) => {
    db.serialize(async function () {
      db.run(
        "INSERT INTO Users (name, type, email, password, state) VALUES (?,?,?,?,?)",
        [name.toUpperCase(), type, email, data, "active"],
        callbackErrorPostData.isError
      );
    ...
    db.all("SELECT * FROM Config", function (err, row) {
      if (row[0]) {
        if (companyLogo) {
          ...
          db.run(
            "UPDATE Config SET companyLogo = ? WHERE id = 1",
        ...
        if (browserLogo) {
          ...
          db.run(
            "UPDATE Config SET browserLogo = ? WHERE id = 1",
        ...
        db.run(
          "INSERT INTO Config (nameCompany, browserLogo, companyLogo) VALUES (?,?,?)",
          [
            nameCompany,
            eliminarHastaRoot(pathBrowserLogo),
            eliminarHastaRoot(pathCompanyLogo),
...
app.post("/postTypeOt", (req, res) => {
  ...
  db.serialize(async function () {
    db.run(
      "INSERT INTO TypeOt (nameType, activities, abbreviation) VALUES (?,?,?)",
      [nameType, JSON.stringify(activities), abbr],
      callbackErrorPostData.isError
    );
  });
...
app.post("/postActivity", (req, res) => {
  ...
  db.serialize(async function () {
    db.run(
      "INSERT INTO Activities (name, score, time, users, state) VALUES (?,?,?,?,?)",
      [name, score, time, "[]", "Created"],
      callbackErrorPostData.isError
    );
  });
});
app.post("/postContract", (req, res) => {
  ...
  db.serialize(async function () {
    db.run(
      "INSERT INTO Contract (name, url) VALUES (?,?)",
      [name, eliminarHastaRoot(contractFilePath)],
      callbackErrorPostData.isError
    );
...
app.post("/postPay", (req, res) => {
  ...
  db.serialize(async function () {
    db.run(
      "INSERT INTO Factura (dateCreated ,dateExpiration ,id, state, datePay) VALUES (?,?,?,?,?)",
      [pay.dateCreated, pay.dateExpiration, pay.id, "Created", pay.datePay],
      callbackErrorPostData.isError
    );
...
app.post("/postOTJson", (req, res) => {
  ...
  db.serialize(async function () {
    for (const otData of otsData) {
      ...
      db.run(
        "INSERT INTO OT (Factura, Client, Date,  Producto, Marca, Modelo, NormaAplicar, Cotizacion, FechaVencimiento, FechaEstimada, Type, Description,StateProcess, Observations, Contact, Changes, Auth, Activities, IdClient, OTKey) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        [
          null,
          OT.Client,
          OT.Date,
...
app.post("/editContract", (req, res) => {
  ...
  db.serialize(async function () {
    if (contractFile) {
      ...
      db.run(
        "UPDATE Contract SET name = ?, url = ? WHERE id = ?",
        [name, eliminarHastaRoot(contractFilePath), id],
        callbackErrorPostData.isError
      );
      res.status(200).json({ result: "ok Contract" });
      return;
...
app.post("/editUser", (req, res) => {
  ...
  db.serialize(async function () {
    if (!password[0]) {
      db.run(
        "UPDATE Users SET name = ?, type = ?, email = ?, state = ? WHERE id = ?",
        [name.toUpperCase(), type, email, state, id]
      );
    } else {
      encryptPassword(password).then((encrypted) => {
        db.run(
          "UPDATE Users SET password = ?, name = ?, type = ?, email = ?, state = ? WHERE id = ?",
...
app.post("/editUsers", (req, res) => {
  ...
  db.serialize(async function () {
    state.forEach((element) => {
      let UsersJson =
        element.otAssign !== null ? JSON.parse(element.otAssign) : { data: [] };
      if (element.state) {
      ...
      db.run("UPDATE Users SET otAssign = ? WHERE id = ?", [
        UsersString,
        element.id,
      ]);
    });
...
app.post("/editOtChanges", (req, res) => {
  ...
  db.serialize(async function () {
    db.get("SELECT Changes FROM OT WHERE id = ?", [idOt], function (err, row) {
      ...
      db.run("UPDATE OT SET Changes = ? WHERE id = ?", [
        ChangesPrevString,
        idOt,
      ]);
...
app.post("/editOt", (req, res) => {
  ...
  db.serialize(async function () {
    db.run(
      "UPDATE OT SET contractName = ?, nLacre = ?, Factura = ?, Availability = ?, IdClient = ?, Activities = ?, Auth = ?,Contact = ?,Observations = ?,StateProcess = ?,Description = ?,Type=?, FechaEstimada = ?, FechaVencimiento = ?,Cotizacion = ?,NormaAplicar = ?,Modelo = ?,Marca = ?,Producto = ?,date=?, Client = ?, OTKey = ?, priority = ? WHERE id = ? ",
      [
        contractName,
        nLacre,
        Factura,
...
app.post("/editOtActivities", (req, res) => {
  ...
  db.serialize(async function () {
    db.run("UPDATE OT SET Activities = ? WHERE id = ? ", [
...
app.post("/editStateActivityOT", (req, res) => {
  const { id, activity, newState } = req.body;
  db.serialize(async function () {
    db.all("SELECT * FROM Ot WHERE OTKey = ?", [id], function (err, row) {
      const activities = JSON.parse(row[0].Activities).map((thisActivity) => ({
        ...thisActivity,
        state: thisActivity.name === activity ? newState : thisActivity.state,
      }));
      db.run("UPDATE OT SET Activities = ? WHERE OTKey = ? ", [
...
app.post("/editOtDescription", (req, res) => {
  ...
  db.serialize(async function () {
    db.run("UPDATE OT SET Description = ? WHERE id = ? ", [
...
app.post("/editOtAvailability", (req, res) => {
  ...
  db.serialize(async function () {
    db.run("UPDATE OT SET Availability = ? WHERE id = ? ", [
...
app.post("/editOtPay", (req, res) => {
  ...
  db.serialize(async function () {
    db.run("UPDATE OT SET Factura = ? WHERE id = ? ", [
      JSON.stringify(pay.newList),
      id,
    ]);
    res.status(200).json({ result: "ok" });
  });
...
app.post("/editTypeOt", (req, res) => {
  ...
  const stringSql = `UPDATE TypeOt SET ${
    nameType ? "nameType = ?," : ""
  } activities = ? ${abbr ? ", abbreviation = ?" : ""}WHERE id = ? `;
  const dataToSend = [nameType, JSON.stringify(activities), abbr, id].filter(
    (item) => item !== null
  );
  db.serialize(async function () {
...
app.post("/editPay", (req, res) => {
  ...
  db.serialize(async function () {
    if (!prevId) {
      ...
      db.run(
        "UPDATE Factura SET datePay = ?,dateCreated = ?,dateExpiration = ?, id = ? WHERE id = ? ",
...
app.post("/editClient", (req, res) => {
  let { Name, Document, KeyUnique, Contacts, id, location, idEditable } =
    req.body;
  db.serialize(async function () {
    db.run(
      "UPDATE Clients SET Name = ?,Document = ?,KeyUnique = ?,location = ?,idEditable=?, Contacts = ? WHERE id = ? ",
...
app.post("/deleteTypeOt", (req, res) => {
  const { id } = req.body;
  db.serialize(async function () {
    db.run("DELETE FROM TypeOt WHERE id = ?", [id]);
  });
...
app.post("/deleteClient", (req, res) => {
  ...
  try {
    db.serialize(async function () {
      db.run("DELETE FROM Clients WHERE idEditable = ?", [id]);
    });
    res.status(200).json({ result: "ok delete" });
  } catch (error) {
    console.log(error);
...
app.post("/deleteContract", (req, res) => {
  ...
  try {
    ...
    db.serialize(async function () {
      db.run("DELETE FROM Contract WHERE id = ?", [id]);
    });
    res.status(200).json({ result: "ok delete" });
  } catch (error) {
    console.log(error);
          ...
          .then((state) => {
            state
              ? res
                  ...
                  .status(200)
                  .json({ result: "ok login", hash: row.password, user: row })
              : res
                  .setHeader("Access-Control-Allow-Origin", "*")
                  .status(200)
                  .json({ result: "error password" });
          })
...
function addSuggObservations(Observations) {
  db.serialize(async function () {
    db.run(
      "INSERT INTO Suggestions (nameType, activities, abbreviation) VALUES (?,?,?)",
      [nameType, JSON.stringify(activities), abbr],
      callbackErrorPostData.isError
    );
  });
...
const clientList = [
  ...
  },
  {
    N: 660,
    CLIENTE:
      "COOP. ELEC. Y DE OBRAS Y SERV. P\u00daBL. LTD DE JUSTINIANO POSSE",
    CUIT: "30-54573491-1",
  },
  {
    N: 663,
  ...
  },
  {
    N: 806,
    CLIENTE:
      "INGENIER\u00cdA EN CONTROL ELECTR\u00d3NICO Y MEC\u00c1NICO S.A. DE C.V.",
    CUIT: "ICE020429672",
  },
  {
    N: 807,
...
const clientListBack = [
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "RUTA PANAMERICANA KM 47.5 - PLANTA IND. Nº 2, Buenos Aires, RAMAL ESCOBAR",
  },
  {
    id: 20,
    Name: "KOMPUSUR SA",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "ROBERTO IMPERIALE 1111 - PARQUE INDUSTRIAL, Santa Fe, VENADO TUERTO",
  },
  {
    id: 26,
    Name: "ATOMLUX SRL",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "COMBATE DE MONTEVIDEO 1139/1141 1139, Tierra del Fuego, RIO GRANDE",
  },
  {
    id: 87,
    Name: "HPM SA",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "AV. CORRIENTES 6041, PISO 9 DPTO. B, Buenos Aires, CAPITAL FEDERAL",
  },
  {
    id: 129,
    Name: "KARULIS SRL",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "AV. CORRIENTES 2312, PISO 3 OF. 31, Buenos Aires, CAPITAL FEDERAL",
  },
  {
    id: 131,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "AV. JUAN DE GARAY 737 PISO 6 DPTO. 3 T: R, Buenos Aires, C.A.B.A.",
  },
  {
    id: 160,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "CALLE 5 ENTRE 2 Y 4 - PARQUE INDUSTRIAL, Buenos Aires, MAR DEL PLATA",
  },
  {
    id: 180,
    Name: "MIMET",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "AV. GAL.PLINIO TOURINHO, 3200, Paraná, Brasil, Río Negro, Paraná, Brasil",
  },
  {
    id: 191,
    Name: "CIROCO SRL",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "ING. AGR. FÉLIX ALDO MARRONE 746 CD. UNIVERSITARIA, Córdoba, CORDOBA",
  },
  {
    id: 210,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "AV. PRESIDENTE PERON 2300 PISO 2 DPTO 3 T 1, Tucumán, YERBA BUENA",
  },
  {
    id: 214,
    Name: "GARMAT S.A.",
    ...
    Contacts: "",
    location:
      "AV. PRESIDENTE PERON 2300 PISO 2 DPTO 3-A T 1, Tucumán, YERBA BUENA",
  },
  {
    id: 215,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Esfuerzo Nº 1-C  -  Col. Lázaro Cárdenas, Edo. de México - MEX, Naucalpan de Juárez",
  },
  {
    id: 217,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "CULIACAN 71 (CANIETI) AV. NUEVO LEON Nº253-802, MEXICO, MEXICO D.F",
  },
  {
    id: 226,
    Name: "ELDEN S.A.",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "RUTA NACIONAL Nº 38 1017 M:143 BARRIO INDUSTRIAL, La Rioja, Chamical",
  },
  {
    id: 241,
    Name: "TOTAL HOME S.A.",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "DR. NICOLAS REPETTO 3656; 4º PISO, Buenos Aires, OLIVOS, VICENTE LÓPEZ",
  },
  {
    id: 267,
    Name: "INNARTEC S.A.",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Av. Universidad s/n Col. La Palma, Edo. de México, MEX, CUAUTITLAN",
  },
  {
    id: 283,
    ...
    Contacts: "",
    location:
      "URBINA 14 COL. PARQUE INDUSTRIAL NAUCALPAN, Edo. de México, MEX, Naucalpan de Juárez",
  },
  {
    id: 284,
    ...
    Contacts: "",
    location:
      "JACINTO CANEK 15-A, COL. SAN JUAN IXHUATEPEC, Edo. de México, MEX, TLANEPANTLA",
  },
  {
    id: 285,
    ...
    Contacts: "",
    location:
      "JOSÉ MA. SALVATIERRA 17529 GARITA DE OTAY, BAJA CALIFORNIA, MEX, TIJUANA",
  },
  {
    id: 286,
    Name: "RP HOGAR SRL",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "MANUEL NICOLÁS CORPANCHO Nº 364, CIUDAD DE MEXICO, COL.  LORENZO BOTURINI",
  },
  {
    id: 291,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Fazenda Farroupilha - Zona Rural, Mato Grosso - BRASIL, Pedra Preta",
  },
  {
    id: 308,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Alem s/n Monoblock 50, 2do Piso Barrio Manantial, San Juan, San Juan",
  },
  {
    id: 311,
    Name: "DAMFER S.R.L.",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Cafetal Nº 139 B Col. Granjas México, Ciudad de México, Iztacalco",
  },
  {
    id: 328,
    Name: "SIEMENS S.A.",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "TRES ARROYOS 2046 - BARRIO VILLA GRAL MITRE, Buenos Aires, C.A.B.A.",
  },
  {
    id: 334,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "RUTA NACIONAL 38 KM 1304 - BARRIO EL PANTANILLO, Catamarca, Sn Fdo del Valle de Catamarca",
  },
  {
    id: 359,
    ...
    Contacts: "",
    location:
      "RUTA 38 1315 ÁREA IND. EL PANTANILLO, Catamarca, Sn Fdo del Valle de Catamarca",
  },
  {
    id: 360,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "AV. DR. H. PUEYRREDON 1548 PISO 2, Buenos Aires, CAPITAL FEDERAL",
  },
  {
    id: 367,
    Name: "HIDROCOR S.R.L.",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "MARIANO ESCOBEDO 164 PB, COL. BARRIO SAN PEDRO, CIUDAD DE MÉXICO, IZTAPALAPA",
  },
  {
    id: 374,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Av. Monteagudo 2121 Entre las calles: Herrera - Re, Misiones, POSADAS",
  },
  {
    id: 379,
    Name: "INNOVAR S.R.L.",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Av. Peñón Texcoco Manzana 750, Lote 26, Estado de MX, Ciudad Nezahualcóyotl",
  },
  {
    id: 401,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Darwin Passaponti 6602, Pque. Ind. Del Oeste, Buenos Aires, Moreno",
  },
  {
    id: 430,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Bvd. Del Mirador 290, Piso 5 Dto. 09, Nordelta, Buenos Aires, TIGRE",
  },
  {
    id: 434,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Av. Ricardo Balbín 2457, Buenos Aires, Billinghurst - San Martín",
  },
  {
    id: 448,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Coronel Loza 7198, Pque. Ind. Los Polígonos (U27), Santa Fe, SANTA FÉ",
  },
  {
    id: 450,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Perito Moreno 845, P.I. Canning, Etapa 2, Nave 61B, Buenos Aires, Canning",
  },
  {
    id: 464,
    Name: "VALLS S.A.",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Av. Rafael Nuñez 3513, Dto. 2, Cerro de Las Rosas, Córdoba, CORDOBA",
  },
  {
    id: 471,
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Av. De Circunvalación entre El Tallo y El Fruto,, Buenos Aires, EL TALAR",
  },
  {
    id: 474,
    Name: "CRIOLLO S.A.",
  ...
  },
  {
    ...
    Contacts: "",
    location:
      "Pan. Colectora Este 38781 Esq. Vte. López, Buenos Aires, BENAVIDEZ",
  },
  {
    id: 488,
    Name: "BDL3K S.R.L.",
